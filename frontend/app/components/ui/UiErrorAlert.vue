...
<template>
  <section class="rounded-2xl border border-error/25 bg-error/8 p-4" role="alert" aria-live="polite">
    <div class="flex items-start gap-3">
      ...
      <button type="button" class="grid size-9 shrink-0 place-items-center rounded-xl text-muted hover:bg-white/6 hover:text-ink" aria-label="بستن پیام خطا" @click="$emit('close')"><CinematicIcon name="x" class="size-4" /></button>
    </div>
  </section>
</template>
