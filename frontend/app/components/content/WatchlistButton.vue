...
const saved = computed(() => isInWatchlist(props.id, props.contentType))

async function handleToggle() {
  const previous = saved.value
  try {
    const added = await toggleWatchlist(props.id, props.contentType)
    if (added) notifications.success('به «لیست من» اضافه شد', 'هر وقت خواستی از بخش «لیست من» پیدایش می‌کنی.')
    else if (previous) notifications.info('از «لیست من» برداشته شد', 'این عنوان دیگر در فهرست ذخیره‌شده تو نیست.')
    if (props.slug && added !== previous) trackWatchlistAction({ id: props.id, slug: props.slug, type: props.contentType }, added)
  } catch (cause) {
    notifications.notifyError(cause, 'لیست من به‌روز نشد.')
  }
}
</script>
...
<template>
  <button
    ...
    :class="[
    iconOnly ? 'size-11' : compactOnMobile ? 'h-12 w-12 px-0 text-sm sm:h-auto sm:w-auto sm:px-4 sm:py-3' : 'min-h-12 px-4 py-3 text-sm',
    ...
    :aria-pressed="saved"
    :title="saved ? 'حذف از لیست من' : 'افزودن به لیست من'"
    @click="handleToggle"
  >
