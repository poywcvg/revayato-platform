# build-webview-apk.ps1 — build the روایتو WebView wrapper into a signed APK
#
# Usage (from repo root):
#   powershell -ExecutionPolicy Bypass -File tools/build-webview-apk.ps1
#
# Options via env:
#   $env:APP_OUTPUT   output file name (default: "ریویتو-app.apk", placed in webview-app/dist/)
#   $env:GRADLE_ARGS  extra gradle args (e.g. "--stacktrace")
#
# Output: webview-app/dist/<APP_OUTPUT>  (a signed release APK, ~1.2 MB)

$ErrorActionPreference = 'Stop'
$root = Split-Path -Parent $PSScriptRoot
$appDir = Join-Path $root 'webview-app'
...
# SDK pin is project-local (webview-app/local.properties → sdk.dir=C:\Android).
$localProps = Join-Path $appDir 'local.properties'
if (-not (Test-Path $localProps)) {
    Write-Error 'missing webview-app/local.properties (sdk.dir=<sdk path>)'
}
...
$outName = if ($env:APP_OUTPUT) { $env:APP_OUTPUT } else { 'ریویتو-app.apk' }
